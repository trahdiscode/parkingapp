...
# ---------- OPENING ANIMATION ----------
st.markdown(f"""<div id="parkos-splash" style="position: fixed; inset: 0; z-index: 99999; display: flex; flex-direction: column; align-items: center; justify-content: center; background: #05060A; overflow: hidden; transition: opacity 1.2s cubic-bezier(0.16, 1, 0.3, 1), visibility 1.2s;"><div style="position: absolute; inset: 0; background-image: url('data:image/svg+xml,%3Csvg viewBox=%220 0 200 200%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cfilter id=%22noiseFilter%22%3E%3CfeTurbulence type=%22fractalNoise%22 baseFrequency=%220.65%22 numOctaves=%223%22 stitchTiles=%22stitch%22/%3E%3C/filter%3E%3Crect width=%22100%25%22 height=%22100%25%22 filter=%22url(%23noiseFilter)%22/%3E%3C/svg%3E'); opacity: 0.04; pointer-events: none; mix-blend-mode: overlay;"></div><div style="position: absolute; width: 60vw; height: 60vw; border-radius: 50%; background: radial-gradient(circle, rgba(124,58,237,0.08) 0%, transparent 60%); top: -20%; left: -10%; filter: blur(90px); animation: float-orb 10s ease-in-out infinite alternate; pointer-events: none;"></div><div style="position: absolute; width: 50vw; height: 50vw; border-radius: 50%; background: radial-gradient(circle, rgba(34,211,238,0.06) 0%, transparent 60%); bottom: -10%; right: -10%; filter: blur(90px); animation: float-orb 12s ease-in-out infinite alternate-reverse; pointer-events: none;"></div><div style="position: relative; z-index: 10; display: flex; flex-direction: column; align-items: center;"><div style="position: relative; opacity: 0; transform: translateY(30px) scale(0.9); filter: blur(10px); animation: premium-reveal 1.4s cubic-bezier(0.16, 1, 0.3, 1) 0.2s forwards;"><div style="position: absolute; inset: 0; background: rgba(124,58,237,0.4); filter: blur(30px); border-radius: 50%; z-index: 0;"></div><img src="data:image/png;base64,{logo_base64}" style="width: 88px; height: 88px; object-fit: contain; position: relative; z-index: 2; filter: drop-shadow(0 8px 24px rgba(0,0,0,0.4));"></div><div style="text-align: center; margin-top: 28px; opacity: 0; transform: translateY(20px); animation: premium-reveal 1.4s cubic-bezier(0.16, 1, 0.3, 1) 0.4s forwards;"><div style="font-family: 'Outfit', sans-serif; font-size: clamp(2.5rem, 6vw, 4rem); font-weight: 800; letter-spacing: -0.04em; line-height: 1; background: linear-gradient(180deg, #FFFFFF 0%, #9CA3AF 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">ParkOS</div><div style="font-family: 'Outfit', sans-serif; font-size: 0.72rem; font-weight: 500; letter-spacing: 0.25em; text-transform: uppercase; color: #6B7280; margin-top: 12px;">Faculty Parking Portal</div></div><div style="margin-top: 48px; width: 160px; display: flex; flex-direction: column; align-items: center; opacity: 0; animation: fade-in 1s ease 1s forwards;"><div style="width: 100%; height: 1px; background: rgba(255,255,255,0.08); position: relative; overflow: hidden; border-radius: 2px;"><div style="position: absolute; top: 0; left: 0; height: 100%; width: 0%; background: linear-gradient(90deg, transparent, #fff, transparent); box-shadow: 0 0 10px rgba(255,255,255,0.5); animation: premium-load 2.2s cubic-bezier(0.65, 0, 0.35, 1) 1.2s forwards;"></div></div><div id="splash-status" style="font-family: 'Outfit', sans-serif; font-size: 0.65rem; color: #6B7280; letter-spacing: 0.05em; margin-top: 12px; transition: opacity 0.3s;">Authenticating...</div></div></div><button onclick="dismissParkOSSplash()" style="position: fixed; bottom: 32px; right: 32px; font-family: 'Outfit', sans-serif; font-size: 0.75rem; font-weight: 500; letter-spacing: 0.05em; color: #9CA3AF; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border-radius: 99px; padding: 8px 20px; cursor: pointer; opacity: 0; transform: translateY(10px); animation: premium-reveal 1s cubic-bezier(0.16, 1, 0.3, 1) 2.2s forwards; transition: all 0.3s ease; z-index: 9999;" onmouseover="this.style.background='rgba(255,255,255,0.08)'; this.style.color='#fff';" onmouseout="this.style.background='rgba(255,255,255,0.03)'; this.style.color='#9CA3AF';">Skip</button></div><style>@keyframes float-orb {{ 0% {{ transform: translate(0, 0) scale(1); }} 100% {{ transform: translate(5%, 5%) scale(1.1); }} }} @keyframes premium-reveal {{ 0% {{ opacity: 0; transform: translateY(30px) scale(0.95); filter: blur(10px); }} 100% {{ opacity: 1; transform: translateY(0) scale(1); filter: blur(0); }} }} @keyframes fade-in {{ to {{ opacity: 1; }} }} @keyframes premium-load {{ 0% {{ left: -100%; width: 50%; }} 100% {{ left: 100%; width: 100%; }} }}</style><script>(function() {{ const statuses = ["Authenticating...", "Securing connection...", "Initializing interface...", "Ready"]; let step = 0; const statusEl = document.getElementById("splash-status"); const statusTimer = setInterval(() => {{ step++; if (step < statuses.length) {{ statusEl.style.opacity = 0; setTimeout(() => {{ statusEl.innerText = statuses[step]; statusEl.style.opacity = 1; }}, 300); }} else {{ clearInterval(statusTimer); }} }}, 700); window.dismissParkOSSplash = function() {{ const splash = document.getElementById("parkos-splash"); if (splash) {{ splash.style.opacity = "0"; splash.style.visibility = "hidden"; setTimeout(() => splash.remove(), 1200); }} }}; setTimeout(window.dismissParkOSSplash, 3800); }})();</script>""", unsafe_allow_html=True)
